    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="w-full max-w-2xl rounded-lg bg-white border border-black/10 shadow-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-black/10 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-slate-800">Import Trainees CSV</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <label className="space-y-1 block">
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-md border border-slate-300 outline-none focus:ring-2 focus:ring-slate-300 file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:bg-slate-200 file:font-semibold file:text-slate-700"
              disabled={submitting}
              required
            />
          </label>
          ...
          {replaceOldBatches ? (
            <label className="space-y-1 block">
              <span className="text-sm font-semibold text-slate-700">Confirmation *</span>
              <input
                type="text"
                value={confirmText}
                ...
                required
              />
              <p className="text-xs text-slate-500">Type exactly: {expectedConfirm}</p>
            </label>
          ) : null}
          ...
          {result ? (
            <div className="space-y-3 rounded-md border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-xs font-semibold text-slate-700">
              ...
              <div className="text-xs font-semibold text-slate-700">
                Imported Batch: <span className="text-slate-900">{result.batch.batch_code}</span>
              </div>

              <div>
                <h4 className="text-xs font-bold text-slate-700 mb-2">Row Errors</h4>
                  ...
                  {result.row_errors.length === 0 ? (
                    ...
                    <ul className="divide-y divide-slate-100">
                      {result.row_errors.map((rowError) => (
                        <li key={`${rowError.row}-${rowError.email}-${rowError.error}`} className="px-3 py-2 text-xs">
