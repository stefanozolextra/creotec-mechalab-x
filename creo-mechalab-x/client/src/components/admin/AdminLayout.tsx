      ...
      <div className="h-screen w-full flex overflow-hidden font-sans select-none transition-colors duration-300 bg-[#EEF2F6] dark:bg-[#0F172A] text-slate-800 dark:text-slate-200">
        ...
        >
          ...
          <div className="h-[100px] flex items-center px-6 overflow-hidden shrink-0">
            <div className="flex items-center gap-4 min-w-[210px]">
              <div className="h-10 w-10 rounded-xl bg-[#0B1B3D] dark:bg-[#0F172A] shrink-0 grid place-items-center">
                <div className="w-4 h-4 rounded bg-cyan-400 dark:bg-[#3B82F6]"></div>
              </div>
          ...
          <div className="p-4 space-y-4 mb-4">
            {/* Theme Toggle Slider */}
            <div className={`relative h-11 flex items-center rounded-full transition-all duration-500 ${isCollapsed ? 'w-11 mx-auto' : 'w-full px-1'} ${isDark ? 'bg-[#0F172A] border border-white/5' : 'bg-[#D1DCE8] shadow-inner'}`}>
              {isCollapsed ? (
                ...
                <>
                  <div className={`absolute left-1 top-1 h-[calc(100%-8px)] w-[calc(50%-4px)] rounded-full transition-transform duration-300 bg-white dark:bg-[#1E293B] shadow-sm ${isDark ? 'translate-x-full' : 'translate-x-0'}`} />
                  <button onClick={() => setIsDark(false)} className={`relative z-10 flex-1 flex items-center justify-center gap-2 h-full font-bold text-sm ${!isDark ? 'text-[#0B1B3D]' : 'text-slate-500'}`}>
                    <Sun size={16} /> Light
                  </button>
                  <button onClick={() => setIsDark(true)} className={`relative z-10 flex-1 flex items-center justify-center gap-2 h-full font-bold text-sm ${isDark ? 'text-slate-100' : 'text-slate-500'}`}>
                    <Moon size={16} /> Dark
                  </button>
                </>
              )}
            ...
            {/* Profile/Logout */}
            <div onClick={() => { clearAuthRole(); navigate('/login'); }} className="flex items-center h-14 rounded-2xl hover:bg-white/40 dark:hover:bg-white/5 transition-all cursor-pointer group overflow-hidden">
              <div className="w-[56px] flex items-center justify-center shrink-0">
